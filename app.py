...
def transcribe():
    try:
        ...
        response = requests.post('https://api.deepgram.com/v1/listen?model=nova-2&smart_format=true', headers=headers, files=files, timeout=300)
...
def analyze():
    try:
        ...
        response = requests.post('https://api.anthropic.com/v1/messages', headers={'Content-Type': 'application/json'}, json={'model': 'claude-sonnet-4-20250514', 'max_tokens': 3000, 'messages': [{'role': 'user', 'content': f'Expert forensic behavioral analyst. Patterns:\n{DECEPTION_PATTERNS}\n\nAnalyze: 1)Deception 2)Timeline 3)Shifts 4)Contacts 5)Bolstering 6)Witnesses 7)Pivoting 8)Patterns 9)Credibility\n\nTranscript:\n{transcript}\n\nExpert format.'}]}, timeout=60)
        if response.status_code != 200:
...
def generate_report():
    try:
        ...
        response = requests.post('https://api.anthropic.com/v1/messages', headers={'Content-Type': 'application/json'}, json={'model': 'claude-sonnet-4-20250514', 'max_tokens': 2500, 'messages': [{'role': 'user', 'content': f'LITIGATION REPORT:\n1)Summary\n2)Findings\n3)Comparison\n4)Credibility\n5)Conclusion\n\n{analysis}'}]}, timeout=60)
        if response.status_code != 200:
            return jsonify({'error': f'Claude error: {response.status_code}'}), 400
